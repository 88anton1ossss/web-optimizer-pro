...
function App() {
  ...
  const handle1ClickExport = async () => {
    if (!selectedAudit || !selectedAudit.ads) return;
    ...
    try {
      ...
      link.setAttribute("download", `Google_Ads_Editor_${domain}.csv`);
      ...
      exportToast.className = "fixed bottom-4 right-4 bg-[#34A853]/10 border border-[#34A853]/30 text-[#34A853] px-6 py-4 rounded-2xl shadow-xl z-[100] flex items-center gap-3 backdrop-blur-xl animate-in slide-in-from-bottom font-black uppercase tracking-widest text-xs";
      exportToast.innerHTML = '<svg class="w-5 h-5 text-[#34A853]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Success! Import CSV to Google Ads Editor';
                    ...
                    <div className="flex items-center gap-2">
                      {isAdsConnected ? (
                        <button
                          ...
                          className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-[#34A853]/30 bg-[#34A853]/10 text-[#34A853] font-black text-xs uppercase tracking-widest hover:bg-[#34A853]/20 transition-all shadow-lg disabled:opacity-50"
                        >
                          {isExportingAds ? (
                          ...
                          {isExportingAds ? "Exporting..." : "1-Click Export to Drafts"}
                        </button>
                      ) : (
                        <button
